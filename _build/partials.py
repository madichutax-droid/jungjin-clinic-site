...
FAX      = "070-7507-5923"
BIZNO    = "880-46-01214"
# 진료 안내문. 2026-09-17 원장님 지시로 **비워 두었습니다** — 18개 페이지에
# 같은 문장이 반복돼 눈에 거슬린다고 하셨습니다.
#
# 되살리려면 아래 한 줄에 문장을 다시 넣으십시오. 그러면 18곳에 한 번에
# 돌아옵니다. content_home.NOTICE 와 content_specialty.NOTICE 도 이 값을
# 가져다 쓰므로 여기 하나만 고치면 됩니다.
#
#   "상태와 원인에 따라 접근이 달라질 수 있으며, 정확한 진단이 우선입니다."
#
# 치료 후기의 면책 줄('작성자 개인의 경험입니다')은 이것과 다른 문장이라
# 그대로 남아 있습니다. 그쪽은 지우지 마십시오.
NOTICE_LINE = ""


def note(text=None):
    """본문 끝 안내문 한 줄. 비어 있으면 빈 <p> 자국도 남기지 않습니다.

    NOTICE_LINE 이 비어 있는 동안은 어느 페이지에서도 나오지 않습니다."""
    t = NOTICE_LINE if text is None else text
    if not t:
        return ""
    return f'      <p class="programs__note">{t}</p>\n'


def notice():
    """NOTICE_LINE 한 줄. note() 의 짧은 이름."""
    return note()
...
def _channel_links():
    ...
    **지우지 않고 옮긴 이유** — 오른쪽 바로가기 바는 641~1399px 에서 숨습니다
    ...
    블로그는 2026-09-01 에 "퀵메뉴에 있으니 푸터에서는 빼라"는 지시로
    뺐다가, 2026-09-22 에 다시 넣었습니다(원장님 지시). 인스타·유튜브가
    여기 생기면서 블로그만 빠져 있으면 오히려 어긋나 보입니다.
    """
    가진것 = [(BLOG, "블로그"), (PLACE, "네이버 플레이스"),
              (INSTA, "인스타그램"), (YOUTUBE, "유튜브")]
...
KAKAO_MAP_KEY = "8cc0ce0543a447f3fdb35fe5e76ebc64"

# 지오코딩에 넣는 주소. 건물명과 층을 빼야 검색이 정확합니다.
KAKAO_MAP_ADDR = "경기 구리시 경춘로 223"
# 첫 화면을 여기에 맞춰 놓고, 주소 검색 결과가 오면 그 자리로 옮깁니다.
# 2026-09-18 카카오 지오코더가 돌려준 명동빌딩 좌표입니다
# (도로명 '경기 구리시 경춘로 223' · 지번 인창동 280-4 · 건물명 명동빌딩).
# 주소 검색이 실패해도 이 값이 있어 핀은 건물 위에 섭니다.
KAKAO_MAP_FALLBACK = "37.6008958, 127.1401666"
