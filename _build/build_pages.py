...
""" + P.tail(with_cta=False))
    ...
    def legal(slug, label, intro, sections):
        # 제목을 ("id", "글") 로 주면 앵커가 붙습니다 — 가입 화면에서
        # '국외 이전' 조항으로 곧장 보내야 해서 넣었습니다.
        arts = []
        for h, paras, lis in sections:
            if isinstance(h, tuple):
                arts.append(f'        <h2 id="{h[0]}">{h[1]}</h2>')
            else:
...
""" + P.tail(with_cta=False))
    ...
    pages["privacy"] = legal(
        ...
        [("1. 수집하는 개인정보 항목",
          ["홈페이지를 둘러보는 것만으로는 개인정보가 수집되지 않습니다. "
           "개인정보는 아래의 경우에만 수집됩니다."],
          ["전화로 진료를 예약하실 때 — 성명, 연락처",
           "내원하여 진료를 받으실 때 — 의료법에 따라 작성되는 진료기록",
           "치료 후기 회원으로 가입하실 때 — 아이디, 비밀번호, 이름, 휴대폰 번호, "
           "생년월일, 이메일 주소"]),

         (("sensitive", "1-1. 치료 후기에 실리는 건강정보"),
          ["치료 후기에 담긴 증상·병력·치료 경과는 개인정보 보호법 제23조의 "
           "<strong>민감정보</strong>에 해당합니다.",
           "치료 후기는 <strong>본원이 게시합니다.</strong> 회원이 직접 글을 쓰는 "
           "기능은 두고 있지 않으며, 회원가입만으로는 건강에 관한 정보가 "
           "수집되지 않습니다.",
           "후기는 해당 환자분께 <strong>서면으로 동의를 받은 내용만</strong> 게시하며, "
           "성명·연락처 등 개인을 특정할 수 있는 정보는 싣지 않습니다. "
           "동의는 언제든 철회하실 수 있고, 철회하시면 해당 글을 지체 없이 내립니다."], []),

         ("2. 개인정보의 이용 목적", ["수집한 정보는 다음 목적으로만 이용하며, 그 밖의 용도로 쓰지 않습니다."],
          ["진료 예약 확인 및 안내", "진료 및 상담 제공", "법령에 따른 진료기록 보존",
           "치료 후기 열람에 필요한 회원 확인",
           "가입하신 분이 실제로 진료를 받으신 분인지 확인하는 일 — 치료 후기는 "
           "의료법에 따라 회원에게만 공개하므로, 이름과 연락처로 확인합니다",
           "아이디나 비밀번호를 잊으셨을 때 본인 확인"]),

         ("3. 보유 및 이용 기간",
          ["관계 법령에서 정한 기간 동안 보관하며, 기간이 지나거나 목적이 달성되면 지체 없이 파기합니다.",
           "진료기록부는 의료법 시행규칙 제15조에 따라 10년간 보관합니다. "
           "예약을 위해 받은 연락처는 예약 목적이 끝나면 파기합니다.",
           "회원 정보는 회원 자격을 유지하시는 동안 보관하고, 탈퇴를 요청하시면 "
           "지체 없이 파기합니다. 생년월일은 같은 이름을 쓰시는 분을 구분하는 "
           "데에만 쓰고, 그 밖의 용도로 쓰지 않습니다."], []),
         ...
         ("5. 개인정보의 제3자 제공 및 처리 위탁",
          ["본원은 환자분의 개인정보를 제3자에게 제공하지 않습니다. "
           "다만 법령에 따른 요구가 있거나 정보주체의 생명·신체의 급박한 위험을 막기 위해 "
           "필요한 경우는 예외로 합니다.",
           "치료 후기 회원 정보의 보관과 인증에 한하여 아래에 처리를 위탁하고 있습니다. "
           "진료기록은 위탁 대상이 아니며 본원 밖으로 나가지 않습니다."],
          ["수탁자 — Supabase, Inc.",
           "위탁 업무 — 회원 계정 인증과 회원 정보의 보관",
           "보유 기간 — 회원 탈퇴 또는 위탁 계약 종료 시까지"]),

         (("abroad", "5-1. 개인정보의 국외 이전"),
          ["치료 후기 회원으로 가입하시면 아래와 같이 개인정보가 국외로 이전됩니다.",
           "회원가입 계약을 이행하기 위해 필요한 보관이므로, 개인정보 보호법 "
           "제28조의8 제1항 제3호에 따라 아래 사항을 이 방침에 공개하는 것으로 "
           "별도 동의를 갈음합니다.",
           "이전을 원하지 않으시면 회원가입을 하지 않으시면 됩니다. "
           "홈페이지의 나머지 기능과 진료 예약은 회원 여부와 관계없이 그대로 "
           "이용하실 수 있습니다."],
          ["이전받는 자 — Supabase, Inc.",
           "이전되는 국가 — 일본 (도쿄 리전)",
           "이전 항목 — 아이디, 암호화된 비밀번호, 이름, 휴대폰 번호, 생년월일, 이메일 주소",
           "이전 일시와 방법 — 회원가입 시점에 정보통신망을 통해 전송",
           "이전 목적 — 회원 계정 인증",
           "보유 기간 — 회원 탈퇴 시까지"]),
         ...
         ("6. 자동으로 수집되는 정보",
          ["본원 홈페이지는 접속자를 추적하거나 분석하는 도구를 쓰지 않으며, "
           ...
           "치료 후기 회원으로 로그인하시면 로그인 상태를 유지하기 위한 인증 토큰이 "
           "브라우저에 저장됩니다. 로그아웃하시면 지워집니다."], []),
         ...
         ("7. 개인정보의 안전성 확보 조치",
          ...
          ["진료기록을 다루는 인원을 최소한으로 제한합니다",
           ...
           "회원 비밀번호는 복호화할 수 없는 방식으로 암호화해 보관하며, 본원도 알 수 없습니다",
           "홈페이지와 서버 사이의 모든 통신은 암호화(HTTPS)됩니다",
           "치료 후기는 로그인한 회원에게만 전송되도록 서버에서 제한하고 있습니다",
           "회원 명부는 본인과 대표원장 외에는 열람할 수 없도록 서버에서 제한하고 있습니다"]),
    ...
    pages["terms"] = legal(
        ...
        [("제1조 (목적)",
         ...
         ("제3조의2 (회원가입과 탈퇴)", [],
          ["회원가입은 아이디·비밀번호·이름·휴대폰 번호·생년월일·이메일 주소로 "
           "신청하며, 신청하시는 즉시 완료됩니다.",
           "이름과 휴대폰 번호를 받는 것은 진료를 받으신 분인지 확인하기 위해서입니다. "
           "치료 후기를 회원에게만 공개하는 이유와 같습니다(아래 제3조의3).",
           "가입 화면의 <strong>가입하기</strong>를 누르시면 본 약관과 "
           "<a href=\"privacy.html\">개인정보처리방침</a>에 동의하신 것으로 봅니다.",
           "회원은 치료 후기를 <strong>열람</strong>하실 수 있습니다. 그 밖의 서비스는 "
           "회원이 아니어도 모두 이용하실 수 있으며, 진료 예약은 회원 여부와 "
           "관계없이 전화로 받습니다.",
           "회원은 언제든지 탈퇴를 요청하실 수 있습니다. 탈퇴하시면 회원 정보가 "
           "지체 없이 삭제됩니다.",
           "아이디나 비밀번호를 잊으신 경우 본원에 전화하시면 본인 확인 후 "
           "다시 알려 드리거나 새로 정해 드립니다.",
           "타인의 정보로 가입하거나 아래 제3조의3을 위반하는 경우, 본원은 "
           "사전에 알린 뒤 회원 자격을 정지하거나 상실시킬 수 있습니다."]),

         ("제3조의3 (치료 후기의 게시와 열람)",
          ["치료 후기는 <strong>의료법 제56조(의료광고의 금지 등)</strong>의 적용을 받습니다. "
           "같은 조 제2항 제2호는 환자의 치료경험담을 광고로 보아 제한하고 있으며, "
           "본원은 이에 따라 치료 후기를 <strong>로그인한 회원에게만</strong> 공개하고 "
           "검색엔진에 노출되지 않도록 하고 있습니다."],
          ["치료 후기는 <strong>본원이 게시합니다.</strong> 회원이 직접 글을 쓰거나 "
           "댓글을 다는 기능은 두고 있지 않습니다.",
           "본원은 해당 환자분께 서면으로 동의를 받은 내용만 게시하며, 성명·연락처 등 "
           "개인을 특정할 수 있는 정보는 싣지 않습니다.",
           "동의는 언제든 철회하실 수 있습니다. 철회 의사를 알려 주시면 해당 글을 "
           "지체 없이 내립니다.",
           "회원은 열람한 내용을 <strong>홈페이지 밖으로 옮겨 싣거나 배포할 수 없습니다.</strong> "
           "회원 전용으로 공개하는 취지가 사라지기 때문입니다."]),
         ...
         ("제7조 (의료 정보에 관한 면책)", [],
          ["홈페이지에 게시된 건강 정보는 <strong>일반적인 정보 제공을 목적</strong>으로 하며, "
           ...
           "치료 후기는 <strong>작성자 개인의 경험</strong>이며, 같은 치료를 받으신 다른 분에게 "
           "같은 결과가 나타난다는 뜻이 아닙니다. 본원의 광고나 치료 효과에 대한 "
           "약속으로 해석되지 않습니다."]),
